...
void RS485_Init(void)
{
        ...
        GPIO_InitStructure.GPIO_Pin=GPIO_Pin_9;//PA2（TX）复用推挽输出
        GPIO_InitStructure.GPIO_Mode=GPIO_Mode_AF_PP;
        GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
        GPIO_Init(GPIOA,&GPIO_InitStructure);
     //   GPIO_SetBits(GPIOA,GPIO_Pin_9);//默认高电平
        
        GPIO_InitStructure.GPIO_Pin=GPIO_Pin_10;//PA3（RX）输入上拉
...
void Modbus_03_Solve(void)
{
        ...
        if((startRegAddr+RegNum)<1000)//寄存器地址+数量在范围内
        {
                ...
                for(i=0;i<RegNum;i++)
                {
                        RS485_TX_BUFF[3+i*2]=0x4d;//(*Modbus_HoldReg[startRegAddr+i]>>8)&0xFF;//              /////////先发送高字节--在发送低字节
                        RS485_TX_BUFF[4+i*2]=0x3e;//(*Modbus_HoldReg[startRegAddr+i])&0xFF; //
									      RS485_TX_BUFF[5+i*2]=0x8d;//(*Modbus_HoldReg[startRegAddr+i]>>8)&0xFF;//              /////////先发送高字节--在发送低字节
                        RS485_TX_BUFF[6+i*2]=0x6e;//(*Modbus_HoldReg[startRegAddr+i])&0xFF; //
                }
                calCRC=CRC_Compute(RS485_TX_BUFF,RegNum*2+3);
                RS485_TX_BUFF[RegNum*2+5]=(calCRC>>8)&0xFF;         //CRC高地位不对吗？  // 先高后低
                RS485_TX_BUFF[RegNum*2+6]=(calCRC)&0xFF;
                RS485_SendData(RS485_TX_BUFF,9);
        }
        else//寄存器地址+数量超出范围
        {
                RS485_TX_BUFF[0]=RS485_RX_BUFF[0];
